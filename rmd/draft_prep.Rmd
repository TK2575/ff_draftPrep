...
budget <- 200
target_points_week <- 131.7 #average winning score in 2021, queried via tk_fantasy_sports repo
dollar_vorp_target_override <- 7.46 #from budget / (130 - manually computed replacement values from 2019 (~101.98))
...
write_csv(replacement_diff, 
          here::here(out, paste0("replacement-diff-", Sys.Date(), ".csv")))

write_csv(replacement_diff, 
          here::here(latest, "replacement-diff-latest.csv"))
```
